...
- Personal info (name, location, job, age, etc.)
- Preferences (likes, dislikes, favorite things)
- Instructions or requests about how to behave
...
- "category": one of "fact", "preference", "instruction"
...
[
  ...
  {"content": "User prefers Korean language responses", "category": "preference"}
]
...
async def extract_memories_background(
    messages: list[dict], model: str, source: str = ""
) -> None:
    ...
    try:
        ...
        if isinstance(items, list) and items:
            add_memories(items, source=source or model)
...
def trigger_memory_extraction(
    ...
    try:
        ...
        if loop.is_running():
            loop.create_task(
                extract_memories_background(messages, model, source)
            )
        else:
            asyncio.run(extract_memories_background(messages, model, source))
    except RuntimeError:
