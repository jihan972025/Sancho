...
**Parameters:**
- `action` (string, required): One of "list", "create", "batch_create", "complete", "delete"
...
- `tasks` (array, for batch_create): Array of task objects, each with `title` (required), `notes` (optional), `due` (optional). **Use batch_create when the user wants to create multiple tasks at once.**
...
```
```
[SKILL_CALL]{"skill": "google_tasks", "params": {"action": "batch_create", "tasks": [{"title": "Outlook에서 DRM파일 열람 관련 보안성심의"}, {"title": "티켓시스템 에러 수정"}, {"title": "카드/커머셜 2026년 2월 통신공사 발주 처리"}]}}[/SKILL_CALL]
```
```
